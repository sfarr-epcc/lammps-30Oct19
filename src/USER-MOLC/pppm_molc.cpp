...
PPPMMolc::PPPMMolc(LAMMPS *lmp) : KSpace(lmp),
  ...
  part2grid(NULL), boxlo(NULL) , ncharges(0)
{
  ...
  memory->create(acons,8,7,"PPPMMolc:acons");
  acons[1][0] = 2.0 / 3.0;
  acons[2][0] = 1.0 / 50.0;
  acons[2][1] = 5.0 / 294.0;
  acons[3][0] = 1.0 / 588.0;
...
{
  if (me == 0) {
    if (screen) fprintf(screen,"PPPMMolc initialization ...\n");
    if (logfile) fprintf(logfile,"PPPMMolc initialization ...\n");
  }
...
#pragma omp parallel for default(none) firstprivate(nlocal, type)\
  ...
  while (order >= minorder) {
    if (iteration && me == 0)
      error->warning(FLERR,"Reducing PPPMMolc order b/c stencil extends "
  ...
  if (order < minorder) error->all(FLERR, "PPPMMolc order < minimum "
                                   "allowed order");
  if (!overlap_allowed && cgtmp->ghost_overlap())
    error->all(FLERR,"PPPMMolc grid stencil extends "
               "beyond nearest neighbor processor");
...
/* ----------------------------------------------------------------------
   adjust PPPMMolc coeffs, called initially and whenever volume has changed
   ------------------------------------------------------------------------- */
...
void PPPMMolc::setup()
{
  ...
  // z dimension for 3d PPPMMolc is zprd since slab_volfactor = 1.0
...
void PPPMMolc::allocate()
{
  memory->create3d_offset(density_brick,nzlo_out,nzhi_out,nylo_out,nyhi_out,
                          nxlo_out,nxhi_out,"PPPMMolc:density_brick");

  memory->create(density_fft,nfft_both,"PPPMMolc:density_fft");
  memory->create(greensfn,nfft_both,"PPPMMolc:greensfn");
  memory->create(work1,2*nfft_both,"PPPMMolc:work1");
  memory->create(work2,2*nfft_both,"PPPMMolc:work2");
  memory->create(vg,nfft_both,6,"PPPMMolc:vg");

  if (triclinic == 0) {
    memory->create1d_offset(fkx,nxlo_fft,nxhi_fft,"PPPMMolc:fkx");
    memory->create1d_offset(fky,nylo_fft,nyhi_fft,"PPPMMolc:fky");
    memory->create1d_offset(fkz,nzlo_fft,nzhi_fft,"PPPMMolc:fkz");
  } else {
    memory->create(fkx,nfft_both,"PPPMMolc:fkx");
    memory->create(fky,nfft_both,"PPPMMolc:fky");
    memory->create(fkz,nfft_both,"PPPMMolc:fkz");
  }

  if (differentiation_flag == 1) {
    memory->create3d_offset(u_brick,nzlo_out,nzhi_out,nylo_out,nyhi_out,
                            nxlo_out,nxhi_out,"PPPMMolc:u_brick");

    memory->create(sf_precoeff1,nfft_both,"PPPMMolc:sf_precoeff1");
    memory->create(sf_precoeff2,nfft_both,"PPPMMolc:sf_precoeff2");
    memory->create(sf_precoeff3,nfft_both,"PPPMMolc:sf_precoeff3");
    memory->create(sf_precoeff4,nfft_both,"PPPMMolc:sf_precoeff4");
    memory->create(sf_precoeff5,nfft_both,"PPPMMolc:sf_precoeff5");
    memory->create(sf_precoeff6,nfft_both,"PPPMMolc:sf_precoeff6");

  } else {
    memory->create3d_offset(vdx_brick,nzlo_out,nzhi_out,nylo_out,nyhi_out,
                            nxlo_out,nxhi_out,"PPPMMolc:vdx_brick");
    memory->create3d_offset(vdy_brick,nzlo_out,nzhi_out,nylo_out,nyhi_out,
                            nxlo_out,nxhi_out,"PPPMMolc:vdy_brick");
    memory->create3d_offset(vdz_brick,nzlo_out,nzhi_out,nylo_out,nyhi_out,
                            nxlo_out,nxhi_out,"PPPMMolc:vdz_brick");
  }

  // summation coeffs

  order_allocated = order;
  if (!stagger_flag) memory->create(gf_b,order,"PPPMMolc:gf_b");
  memory->create2d_offset(rho1d,3,-order/2,order/2,"PPPMMolc:rho1d");
  memory->create2d_offset(drho1d,3,-order/2,order/2,"PPPMMolc:drho1d");
...
void PPPMMolc::allocate_peratom()
{
...
void PPPMMolc::deallocate_peratom()
{
  peratom_allocate_flag = 0;
...
double PPPMMolc::estimate_ik_error(double h, double prd, bigint ncharges)
{
  double sum = 0.0;
...
/* ----------------------------------------------------------------------
   set local subset of PPPMMolc/FFT grid that I own
...
void PPPMMolc::set_grid_local()
{
  // global indices of PPPMMolc grid range from 0 to N-1
  // nlo_in,nhi_in = lower/upper limits of the 3d sub-brick of
  //   global PPPMMolc grid that I own without ghost cells
  // for slab PPPMMolc, assign z grid as if it were not extended

  // printf("PPPMMolc::set_grid_local subbox %i %f %f %f %f %f %f\n",
  // 	 comm->me,
  ...
  // printf("PPPMMolc::set_grid_local %i sub\n%i %i\n%i %i\n%i %i\n",
  // 	 comm->me,
  // 	 nxlo_in, nxhi_in,
  // 	 nylo_in, nyhi_in,
  // 	 nzlo_in, nzhi_in);

  // nlower,nupper = stencil size for mapping particles to PPPMMolc grid

  nlower = -(order-1)/2;
  nupper = order/2;

  // printf("PPPMMolc::set_grid_local stencil %i %i\n",
  // 	 nlower, nupper);
  ...
  // printf("PPPMMolc::set_grid_local shift %f %f\n",
  // 	 shift, shiftone);

  // nlo_out,nhi_out = lower/upper limits of the 3d sub-brick of
  //   global PPPMMolc grid that my particles can contribute charge to
  ...
  // for slab PPPMMolc, change the grid boundary for processors at +z end
  //   to include the empty volume between periodically repeating slabs
  // for slab PPPMMolc, want charge data communicated from -z proc to +z
  ...
  // PPPMMolc grid pts owned by this proc, including ghosts
...
void PPPMMolc::compute_gf_denom()
{
  int k,l,m;
...
void PPPMMolc::compute_gf_ik_triclinic()
{
  double snx,sny,snz;
  double argx,argy,argz,wx,wy,wz,sx,sy,sz,qx,qy,qz;
  double sum1,dot1,dot2;
...
void PPPMMolc::compute_sf_precoeff()
{
  int i,k,l,m,n;
...
void PPPMMolc::poisson_ik_triclinic()
{
  int i,j,k,n;
...
void PPPMMolc::pack_forward(int flag, FFT_SCALAR *buf, int nlist, int
                                 *list)
{
  int n = 0;
...
void PPPMMolc::unpack_forward(int flag, FFT_SCALAR *buf, int nlist,
                                   int *list)
{
  int n = 0;
...
void PPPMMolc::pack_reverse(int flag, FFT_SCALAR *buf, int nlist,
                                 int *list)
{
  if (flag == REVERSE_RHO) {
...
void PPPMMolc::unpack_reverse(int flag, FFT_SCALAR *buf, int nlist,
                                   int *list)
{
  if (flag == REVERSE_RHO) {
...
void PPPMMolc::procs2grid2d(int nprocs, int nx, int ny, int *px, int *py)
{
...
void PPPMMolc::compute_rho1d(const FFT_SCALAR &dx, const FFT_SCALAR &dy,
                                  const FFT_SCALAR &dz)
{
  int k,l;
  FFT_SCALAR r1,r2,r3;
...
void PPPMMolc::compute_drho1d(const FFT_SCALAR &dx, const FFT_SCALAR &dy,
                                   const FFT_SCALAR &dz)
{
  int k,l;
  FFT_SCALAR r1,r2,r3;
...
void PPPMMolc::compute_rho_coeff()
{
  int j,k,l,m;
  FFT_SCALAR s;

  FFT_SCALAR **a;
  memory->create2d_offset(a,order,-order,order,"PPPMMolc:a");
...
int PPPMMolc::timing_1d(int n, double &time1d)
{
  double time1,time2;
...
int PPPMMolc::timing_3d(int n, double &time3d)
{
  double time1,time2;
...
/* ----------------------------------------------------------------------
   compute the PPPMMolc total long-range force and energy for groups A and
   B
   ------------------------------------------------------------------------- */

void PPPMMolc::compute_group_group(int groupbit_A,
                                        int groupbit_B,
                                        int AA_flag)
{
...
void PPPMMolc::allocate_groups()
{
  group_allocate_flag = 1;

  memory->create3d_offset(density_A_brick,nzlo_out,nzhi_out,nylo_out,nyhi_out,
                          nxlo_out,nxhi_out,"PPPMMolc:density_A_brick");
  memory->create3d_offset(density_B_brick,nzlo_out,nzhi_out,nylo_out,nyhi_out,
                          nxlo_out,nxhi_out,"PPPMMolc:density_B_brick");
  memory->create(density_A_fft,nfft_both,"PPPMMolc:density_A_fft");
  memory->create(density_B_fft,nfft_both,"PPPMMolc:density_B_fft");
}
...
void PPPMMolc::deallocate_groups()
{
  group_allocate_flag = 0;
...
void PPPMMolc::poisson_groups(int AA_flag)
{
  int i,j,k,n;
...
void PPPMMolc::poisson_groups_triclinic()
{
  int i,n;
