...
void PairMolcCut::settings(int narg, char **arg)
{
  ...
  cut_coul_global = force->numeric(FLERR,arg[4]);
...
void PairMolcCut::coeff(int narg, char **arg)
{
  ...
  int i = force->inumeric(FLERR,arg[0]);
  int j = force->inumeric(FLERR,arg[1]);
  ...
  // now set the charges
  int nCoulSites = force->inumeric(FLERR, arg[7]);
  ...
  if ((argcount+2)==narg){
    cut_lj_one =  force->numeric(FLERR, arg[argcount+1]);
    cut_coul_one =  force->numeric(FLERR, arg[argcount+2]);
  }
